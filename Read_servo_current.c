...
int main()
{
    //宣言
        ...
        float IC1,IC2,IC3;        //電流値[mA] Intensity of Current
    ...
    // トルクをONする(トルク ON=1/OFF=0)
       ...
       ret1=RSTorqueOnOff(1,fd,1);
       ret2=RSTorqueOnOff(2,fd,1);
       ret3=RSTorqueOnOff(3,fd,1);
    ...
    //（テスト）電流値を読む
        // IC1=((float)RSGetCurrent(fd,1));   //単位は[mA]
        // IC2=((float)RSGetCurrent(fd,2));
        // IC3=((float)RSGetCurrent(fd,3));

        // printf("\nID1の電流値は%f[mA]\n",IC1);   
        // printf("ID2の電流値は%f[mA]\n",IC2);
        // printf("ID3の電流値は%f[mA]\n\n",IC3);
    //

    //FKより現在異位置を求める(各モータの現在角度を読み出し→FK)
        //現在角度読み出し
            printf("\nRead Servo Anglen\n");
            int i;
